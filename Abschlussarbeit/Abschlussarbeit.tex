...
Shaderunterstützung, mit der sich zum Beispiel Oberflächeneigenschaften
...
Transformation $M$, wird die Kamera über $M^{-1}$ in das
...
Kinderknoten enthalten. Zusätzlich gibt es noch den \texttt{LightNode}. Dieser
repräsentiert die verschiedenen Lichtquellen und wird von 3 Standardlichtquellen
...
Der Betrachter einer Szene wird über eine Kamera dargestellt. Der
...
%TODO: Standards des jVR-Framework Einleitung
...
Erweiterung des jVR-Framework ist dabei als Schwerpunkt zu sehen. Es sollen
nicht nur Portale genutzt werden, sondern auch selbst platziert werden können.
...
verschiedenen Blickwinkeln rendert und auf die jeweilige Portalfläche projeziert
wird. Dass bedeutet, dass jeder von den aufgeführten Portaltypen eine
gemeinsame Oberklasse besitzen und von dieser abgeleitet werden.
...
Bewegungsgeschwindigkeit der Kamera repräsentiert.

Um den Betrachter teleportieren zu können, wird zunächst die
\texttt{update}-Methode aufgerufen. Die übergebene \texttt{CameraNode} wird in
das Objektkoordinatensystem des Portals überführt. Danach wird die
Transformation gespeichert und wiederum mit der Transformation des
\texttt{portalExit} multipliziert. Als letzten Schritt wird die Transformation
um 180° auf der y-Achse gedreht. Die daraus resultierende Transformation wird
auf die virtuellen Kamera angewendet. Die \texttt{update}-Methode ruft zudem die
Methode \texttt{getPickPoint} auf.
...
Fall wird darauf die \texttt{teleport}-Methode aufgerufen. Dies geschieht, indem
der Szeneknoten ins Koordinatensystem des Portals transformiert wird.
Daraufhin wird aus der Transformation der Translations-Vektor gewonnen und mit
den minimalen swoie maximalen Werten der \emph{Bounding Box} verglichen.

Die \texttt{teleport}-Methode nimmt die \texttt{SceneNode} und transformiert sie
ins Objektkoordinatensystem. Aus der gewonnenen Transformation werden die
Transformation für die Translation und die Rotation extrahiert. Danach wir die
Transformation des \texttt{portalExit} mit der Rotationstransformation
verrechnet. Dies wird gemacht, das der Szeneknoten mit der gleichen Rotation aus
dem Teleporterausgang austritt, wie er den Teleportereingang betreten hat.

Danach wird der Knoten noch mit einem Offset in die z-Richtung transformiert um
vor dem Portal und dessen \emph{Bounding Box} zu landen und nicht darin. Wäre
dies der Fall würde der Szeneknoten bei Austritt umgehend wieder teleportiert
werden.
...
Der Vorgang wird nochmals im Gleichung \ref{eq:trans} veranschaulicht. Dabei ist
$N$ die Transformation des Teleporterausgangs, $R$ ist die extrahierte
Rotationstransformation und $T$ die extrahierte Translationstransformation.
$S_{node}$ stellt die Transformation des Szeneknotens beim Teleporteraustritt
dar.

% TODO überarbeiten, nicht richtig!!!!
\begin{equation}
S_{node} = R * T * N
\label{eq:trans}
\end{equation}
...
\texttt{PortalConnector} aufgerufen. Sie stellt die statische Methode
\texttt{connect} zur Verfügung. Ihr werden 2 Teleporter übergeben, die verbunden
werden sollen.
...
Teleporter besitzt eine Klassenvariable \texttt{portalExit} vom Typ CameraNode.
...
\texttt{update}-Methode übergeben. Die Szeneknoten in der \texttt{GroupNode}
werden genau wie die Kamera transformiert und treten danach aus dem
Portalausgang aus. Jeder einzelne Szeneknoten wird überprüft, ob er sich
innerhalb der \emph{Bounding Box} des Teleporters befindet. Die Transformation
wird erst dann angewendet, wenn dies der Fall ist.

\subsection{Mirror-Klasse}

Die \texttt{Mirror}-Klasse ist die Repräsentation eines Spiegel. Sie kann die
Szene reflektieren. 
